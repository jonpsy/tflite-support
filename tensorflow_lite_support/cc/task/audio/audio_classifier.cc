...
using ::tflite::task::core::ClassificationResult;
...
void AudioClassifier::SetInputs() {
  // TODO(b/182625132): Retrieve the required audio format from the model
  // metadata.
  audio_format_.SetChannels(1);
  audio_format_.SetEncodingFormat(AudioFormat::Encoding::kPCMFloat);
...
absl::Status AudioClassifier::Preprocess(
  ...
  tflite::task::core::PopulateTensor(audio_buffer, num_elements,
                                     input_tensors[0]);
  return absl::OkStatus();
}

tflite::support::StatusOr<core::ClassificationResult>
...
absl::Status AudioClassifier::FillResultsFromLabelMaps(
  ...
  for (int i = 0; i < result->classifications_size(); ++i) {
    core::Classifications* classifications = result->mutable_classifications(i);
