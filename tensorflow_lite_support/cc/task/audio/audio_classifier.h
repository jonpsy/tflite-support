...
#include "tensorflow_lite_support/cc/task/audio/core/audio_format.h"
...
#include "tensorflow_lite_support/cc/task/core/proto/classifications_proto_inc.h"
...
// Input tensor:
//   (kTfLiteFloat32)
//    - input audio buffer of size `[frames x channels]`.
...
class AudioClassifier
    : public tflite::task::core::BaseTaskApi<core::ClassificationResult,
                                             const float*> {
 public:
  ...
  // which can retrieved by GetAudioClipFormat().
  tflite::support::StatusOr<core::ClassificationResult> Classify(
      const float* audio_buffer);

  // Returns the expected input audio format.
  // TODO(b/182625132): Add unit test after the format is populated from model
  // metadata.
  AudioFormat GetAudioFormat() { return audio_format_; }
 ...
 private:
  ...
  absl::Status Preprocess(const std::vector<TfLiteTensor*>& input_tensors,
                          const float* audio_buffer) override;

  // Post-processing to transform the raw model outputs into classification
  // results.
  tflite::support::StatusOr<core::ClassificationResult> Postprocess(
  ...
  absl::Status FillResultsFromLabelMaps(core::ClassificationResult* result);
